...
import my_email # from => my_emal.py
...
root.title("Tkinter Soyağaci Giriş Formu")
...
add_student_pic_icon=tk.PhotoImage(file='images/Images/add_image.png')
...
def add_data(id_number, password, name, age, gender, phone_number,
    ...
    cursor.execute(f"""
    INSERT INTO data VALUES('{id_number}', '{password}', '{name}','{age}',
    '{gender}', '{phone_number}', '{student_class}', '{email}', ?)
...
def student_card_page(student_card_obj, bypass_login_page=False):
    ...
    student_card_pages_fm=tk.Frame(root, highlightbackground=bg_color, 
                                  highlightthickness=3)
    
    heading_lb=tk.Label(student_card_pages_fm,
                        text=" Student Card",
                        bg=bg_color, fg="white", font=("Bold", 18))
    heading_lb.place(x=0, y=0, width=400)

    close_btn=tk.Button(student_card_pages_fm, text="x", bg=bg_color,
                            fg="white",font=("Bold", 13), bd=0,
                            command=close_page)
    close_btn.place(x=370, y=0)

    #card_img=ImageTk.PhotoImage(Image.open('images/Images/student_card_frame.png'))

    sudent_card_lb=tk.Label(student_card_pages_fm, image=student_card_img)#, image=card_img)
    sudent_card_lb.place(x=50, y=50)

    sudent_card_lb.image=student_card_img

    save_student_card_btn=tk.Button(student_card_pages_fm, text="Save Student Card", bg=bg_color,
                            fg="white",font=("Bold", 13), bd=1, command=save_student_card)                         
    save_student_card_btn.place(x=80, y=375)

    print_student_card_btn=tk.Button(student_card_pages_fm, text="🖨", bg=bg_color,
                            fg="white",font=("Bold", 15), bd=1, command=print_student_card)                          
    print_student_card_btn.place(x=270, y=370)


    student_card_pages_fm.place(x=50, y=30, width=400, height=450)

def welcome_page():

    def forward_to_student_login_page():
        welcome_pages_fm.destroy()
        root.update()
        student_login_page()
    ...
    welcome_pages_fm=tk.Frame(root, highlightbackground=bg_color,
                            highlightthickness=3)

    heading_lb=tk.Label(welcome_pages_fm,
                        text="Welcome To Student Registration\n&& Managment System",
                        bg=bg_color, fg="white", font=("Bold", 18))
    heading_lb.place(x=0, y=0, width=400)

    student_login_bt=tk.Button(welcome_pages_fm, text="Login Student", bg=bg_color,
                            fg="white",font=("Bold", 15),bd=0,
                            command=forward_to_student_login_page)
    student_login_bt.place(x=120, y=125, width=200)

    student_login_img=tk.Button(welcome_pages_fm, image=login_student_icon, bd=0,
                                command=forward_to_student_login_page)
    student_login_img.place(x=60, y=100)

    admin_login_bt=tk.Button(welcome_pages_fm, text="Login Admin", bg=bg_color,
                            fg="white",font=("Bold", 15), bd=0, 
                            command=forward_to_admin_login_page)
    admin_login_bt.place(x=125, y=225, width=200)

    admin_login_img=tk.Button(welcome_pages_fm, image=login_admin_icon, bd=0,
                              command=forward_to_admin_login_page)
    admin_login_img.place(x=60, y=200)


    add_student_bt=tk.Button(welcome_pages_fm, text="Create Account", bg=bg_color,
                            fg="white",font=("Bold", 15), bd=0,
                            command=forward_to_add_account_page)
    add_student_bt.place(x=120, y=325, width=200)

    add_student_img=tk.Button(welcome_pages_fm, image=add_student_icon, bd=0, 
                              command=forward_to_add_account_page)
    add_student_img.place(x=60, y=300)


    welcome_pages_fm.pack(pady=30)
    welcome_pages_fm.pack_propagate(False)
    welcome_pages_fm.config(width=400, height=420)
...
def forget_password_page():
        def recover_password():
            if check_id_already_exists(id_number=student_id_ent.get()):
                #print("Correct ID: ", student_id_ent.get())
                conntion=sqlite3.connect('students_accounts.db')
...
""")
                ...
                print('recovered_password: ', recovered_password)
                cursor.execute(f"""
...
""")
                ...
                conntion.close()
                confirmation=confirmation_box(f"""
...
Do You Want to Continue?""")
                if confirmation:
                    msg="""
                    <h1>Your Forgot Password is<\h1>
                    <h2>{recovered_password<|h2>
                    <p>Once Remember Your Peassword After Delete this Message<|p>"""
                    sendmail_to_student(email=student_email, message=msg,                        
                                        subject="Pasword Recovery")
            else:
                print("Incorrect ID") 
                message_box(message="Invalid ID Numger")   
       
        forget_password_pages_fm=tk.Frame(root, highlightbackground=bg_color,
                                    highlightthickness=3)
        heading_lb=tk.Label(forget_password_pages_fm,
                            text="⚠ Forgetting Password",
                            bg=bg_color, fg="white", font=("Bold", 15))
        heading_lb.place(x=0, y=0, width=350)  

        close_btn=tk.Button(forget_password_pages_fm, text="x", font=("Bold", 13), bg=bg_color,
                       fg='white', bd=0, command=lambda:forget_password_pages_fm.destroy())
        close_btn.place(x=320, y=0)

        student_id_lb=tk.Label(forget_password_pages_fm,
                            text="Enter Student ID Number",font=("Bold", 13))                   
        student_id_lb.place(x=70, y=40)  

        student_id_ent=tk.Entry(forget_password_pages_fm, font=("Bold", 15),
                        justify=tk.CENTER, highlightbackground="gray",
                        highlightcolor=bg_color, highlightthickness=2)
        student_id_ent.place(x=70, y=70, width=180)

        info_lb=tk.Label(forget_password_pages_fm, text="""
Via Your Email Address
We Will Sent to You
Your Forget Password """, justify=tk.LEFT)                   
        info_lb.place(x=75, y=110) 

        next_btn=tk.Button(forget_password_pages_fm, 
                text="Next", font=("Bold", 13), bg=bg_color,
                fg='white', command=recover_password)
        next_btn.place(x=130, y=200, width=80)

        forget_password_pages_fm.place(x=75, y=120, width=350, height=250)
...
def student_dashboard(student_id):
    ...
    home_btn=tk.Button(option_fm, text="Home",font=("Bond", 15),
    ...
    student_card_btn=tk.Button(option_fm, text="Sdudent\nCard",font=("Bond", 15),
    ...
    student_card_btn_indicator=tk.Label(option_fm, bg='#c3c3c3')
    student_card_btn_indicator.place(x=5, y=108, width=3, height=40)

    security_btn=tk.Button(option_fm, text="Security",font=("Bond", 15),
                       fg=bg_color,bg='#c3c3c3', bd=0,
                       command=lambda:switch(indicator=security_btn_indicator,
                                             page=security_page))
    security_btn.place(x=10, y=170)

    security_btn_indicator=tk.Label(option_fm, bg='#c3c3c3')
    security_btn_indicator.place(x=5, y=170, width=3, height=40)
 
    edit_data_btn=tk.Button(option_fm, text="Edit Data",font=("Bond", 15),
                       fg=bg_color,bg='#c3c3c3', bd=0,
                       command=lambda:switch(indicator=edit_data_indicator,
                                             page=edit_data_page))
    ...
    def home_page():
        ...
        size=100
        mask=Image.new(mode='L',size=(size,size)) # 'L' gray style img
        #mask.show()
...
"""
    ...
    #student_card_image_obj.show()
        ...
        save_student_card_btn=tk.Button(student_card_page_fm, 
        ...
        print_student_card_btn=tk.Button(student_card_page_fm, 
            text='🖨', font=("Bold",15),bd=1, fg='white', 
    ...
    def security_page():
        def show_hide_password():
            ...
            if current_password_ent["show"]=="*":
                ...
                shwo_hide_btn.config(image=unlocked_icon)
            else:
                current_password_ent.config(show="*")
                shwo_hide_btn.config(image=locked_icon)
        ...
        security_pages_fm=tk.Frame(pages_fm)

        current_password_lb=tk.Label(security_pages_fm, text="Your Current Password",
                            font=("Bold",12))
        current_password_lb.place(x=80, y=30)

        current_password_ent=tk.Entry(security_pages_fm, font=("Bold",15),
        ...
        #print("pass:" , student_current_password)

        current_password_ent.insert(tk.END, student_current_password[0][0])
        current_password_ent.config(state='readonly')

        shwo_hide_btn=tk.Button(security_pages_fm, image=locked_icon,bd=0,
                            command=show_hide_password)
        shwo_hide_btn.place(x=280, y=70)

        change_password_lb=tk.Label(security_pages_fm, text="Change Password",
                            font=("Bold",15), bg="red", fg='white')
        
        change_password_lb.place(x=30, y=210, width=290)

        new_password_lb=tk.Label(security_pages_fm, text="Set New Password",
                            font=("Bold",12))
        new_password_lb.place(x=100, y=280)

        new_password_ent=tk.Entry(security_pages_fm, font=("Bold",15),
                                      justify=tk.CENTER, show="*")
        new_password_ent.place(x=60, y=330)

        change_password_btn=tk.Button(security_pages_fm, text="Set New Password", 
                            font=("Bold",12),bg=bg_color,fg="white",
                            command=set_password)
        change_password_btn.place(x=110, y=380)


        security_pages_fm.pack(fill=tk.BOTH, expand=True)  
    ...
    def edit_data_page():
        ...
        def verify_email(email):
            ...
            pattern="^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[_a-z]{2,4})$"
        ...
        def check_inputs():
            ...
            if student_name_ent.get() == '':
                ...
                message_box(message='Student Phone is Required')
                ...
                message_box(message='Data Successfuly Updated. ')
        ...
        edit_data_page_lb=tk.Label(edit_data_page_fm, font=("Bold",15))
        ...
        student_email_lb=tk.Label(edit_data_page_fm, text="Student Emil Address",
                                font=("Bold", 12))
        ...
        update_btn=tk.Button(edit_data_page_fm, text="Update", font=("Bold", 15),
                             fg="white", bg=bg_color, bd=0,
                             command=check_inputs)
        update_btn.place(x=220, y=470, width=80)
    ...
    def delete_account_page():
        ...
        def confirm_delete_account():
            ...
            if confirm:
                ...
                welcome_page()
                root.update()
                message_box(message="Account Successfuly Deleted")


        delete_account_page_fm=tk.Frame(pages_fm)

        dlete_account_lb=tk.Label(delete_account_page_fm, text="⚠ Delete Account",
                            font=("Bold",15), bg="red", fg="white")
        dlete_account_lb.place(x=30, y=100, width=290)
...
def student_login_page():
    ...
    def forward_to_welcom_page():
            student_loging_pages_fm.destroy()
            root.update()
            welcome_page()
    ...
    def login_account():
        ...
        if verify_id_number:
           # print('ID is Correct:', id_number_ent.get() )
            ...
            if verify_password:
                ...
                #print('Password is Correct:', password_ent.get())
                student_loging_pages_fm.destroy()
    ...
    student_loging_pages_fm=tk.Frame(root, highlightbackground=bg_color,
                                highlightthickness=3)

    heading_lb=tk.Label(student_loging_pages_fm,
                            text="Student Loging Page",
                            bg=bg_color, fg="white", font=("Bold", 18))
    heading_lb.place(x=0, y=0, width=400)  

    back_btn=tk.Button(student_loging_pages_fm, text="←", font=("Bold", 20),
                       fg=bg_color, bd=0, command=forward_to_welcom_page)
    back_btn.place(x=5, y=40)


    student_icon_lb=tk.Label(student_loging_pages_fm, image=login_student_icon)
    student_icon_lb.place(x=150, y=40)

    id_number_lb=tk.Label(student_loging_pages_fm, text="Enter Student ID Number",
                        font=("Bold", 15), fg=bg_color)
    id_number_lb.place(x=80, y=140)

    id_number_ent=tk.Entry(student_loging_pages_fm, font=("Bold", 15),
                        justify=tk.CENTER, highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    ...
    password_lb=tk.Label(student_loging_pages_fm, text="Enter Student Password",
                        font=("Bold", 15), fg=bg_color)
    password_lb.place(x=80, y=240)

    password_ent=tk.Entry(student_loging_pages_fm, font=("Bold", 15),
                        justify=tk.CENTER, highlightbackground="gray", 
                        highlightcolor=bg_color, highlightthickness=2,
                        show="*")
    ...
    shwo_hide_btn=tk.Button(student_loging_pages_fm, image=locked_icon,bd=0,
                            command=show_hide_password)
    shwo_hide_btn.place(x=310, y=280)


    login_bt=tk.Button(student_loging_pages_fm, text="Login", bg=bg_color,
                                fg="white",font=("Bold", 15), 
                                command=login_account)
    login_bt.place(x=95, y=340, height=40, width=200)

    foget_password_btn=tk.Button(student_loging_pages_fm, text="⚠\nForget Password",
                                fg=bg_color, bd=0, command=forward_to_forget_password_page)
    foget_password_btn.place(x=150, y=390)


    student_loging_pages_fm.pack(pady=30)
    student_loging_pages_fm.pack_propagate(False)
    student_loging_pages_fm.config(width=400, height=450)
...
def admin_dashboard():
    ...
    options_fm=tk.Frame(dashboard_fm, highlightbackground=bg_color,
                        bg="#c3c3c3", highlightthickness=2)
    ...
    def home_page():
        ...
        home_page_fm=tk.Frame(pages_fr)
...
"""
        ...
        find_student_page_fm=tk.Frame(pages_fr)
        ...
        find_by_lb=tk.Label(find_student_page_fm,
                                        text="Find By ", font=("Bold", 12))
        ...
        record_table_lb=tk.Label(find_student_page_fm,
                                        text="Record Table ", 
                                        font=("Bold", 13), bg=bg_color, fg='white')
    ...
    def announcement_page():
        ...
        def collect_emails():
            ...
            thread.start()
            send_announcement(email_addresses=fetched_emails)
        ...
        def send_announcement(email_addresses):
            ...
            subject=announcement_subject_ent.get()
            sent_count=0
            for email in email_addresses:
                sending_lb.config(f"Sending To:\n{email}\n\n{sent_count}/{len(email_addresses)}")
        ...
        announcement_page_fm=tk.Frame(pages_fr)

        subject_lb=tk.Label(announcement_page_fm,text="Enter Announcement Subject",
                            font=("Bold",12))
        subject_lb.place(x=10, y=10)

        announcement_subject_ent=tk.Entry(announcement_page_fm, font=("Bold", 12))
        announcement_subject_ent.place(x=10, y=40, width=210, height=25)
    ...
    pages_fr=tk.Frame(dashboard_fm)
...
def admin_login_page():
    ...
    admin_login_pages_fm=tk.Frame(root, highlightbackground=bg_color,
                                    highlightthickness=3)

    heading_lb=tk.Label(admin_login_pages_fm,
                                text="Admin Loging Page",
                                bg=bg_color, fg="white", font=("Bold", 18))
    heading_lb.place(x=0, y=0, width=400)

    back_btn=tk.Button(admin_login_pages_fm, text="←", font=("Bold", 20),
                       fg=bg_color, bd=0, command=forward_to_welcome_page)
    back_btn.place(x=5, y=40)

    admin_icon_lb=tk.Label(admin_login_pages_fm, image=login_admin_icon)
    admin_icon_lb.place(x=150, y=40)

    username_lb=tk.Label(admin_login_pages_fm, text="Enter Admin User Name",
                        font=("Bold", 15), fg=bg_color)
    username_lb.place(x=80, y=140)

    username_ent=tk.Entry(admin_login_pages_fm, font=("Bold", 15),
                        justify=tk.CENTER, highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    username_ent.place(x=80, y=190)

    password_lb=tk.Label(admin_login_pages_fm, text="Enter Admin Password",
                        font=("Bold", 15), fg=bg_color)
    password_lb.place(x=80, y=240)

    password_ent=tk.Entry(admin_login_pages_fm, font=("Bold", 15),
                        justify=tk.CENTER, highlightbackground="gray", 
                        highlightcolor=bg_color, highlightthickness=2,
                        show="*")
    password_ent.place(x=80, y=290)

    shwo_hide_btn=tk.Button(admin_login_pages_fm, image=locked_icon, bd=0,
                            command=show_hide_password)
    shwo_hide_btn.place(x=310, y=280)

    login_bt=tk.Button(admin_login_pages_fm, text="Login", bg=bg_color,
                                fg="white",font=("Bold", 15),
                                command=login_account)
    login_bt.place(x=95, y=340, height=40, width=200)


    admin_login_pages_fm.pack(pady=30)
    admin_login_pages_fm.pack_propagate(False)
    admin_login_pages_fm.config(width=400, height=430)
...
def add_account_page():     
    ...
    def check_input_validation():
        ...
        if student_name_ent.get()=="":
            ...
            student_email_ent.config(highlightbackground="red",
                                       highlightcolor="red") 
            student_email_ent.focus()
            message_box(message="Please Enter a Valid\nEmil Address")  
            ...
            message_box(message="Student Password is Required") 
            ...
            if pic_path.get() != '':
                ...
                read_data=open('images/Images/add_image.png', 'rb')
                pic_data=read_data.read()
                read_data.close()
                pic_path.set('images/Images/add_image.png')
            ...
            #message_box("Account Succsefuly Created")
...
"""
    ...
    add_account_pages_fm=tk.Frame(root, highlightbackground=bg_color,
                                        highlightthickness=3)

    add_pic_section_fm=tk.Frame(add_account_pages_fm, highlightbackground=bg_color,
                            highlightthickness=2)
    ...
    student_name_lb=tk.Label(add_account_pages_fm, text="Enter Student Full Name",
                            font=("Bold", 12))
    student_name_lb.place(x=5, y=130)

    student_name_ent=tk.Entry(add_account_pages_fm, font=("Bold", 15),
                            highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    student_name_ent.place(x=5, y=160, width=180)
    ...
    student_gender_lb=tk.Label(add_account_pages_fm, text="Select Student Gender ",
                            font=("Bold", 12))
    student_gender_lb.place(x=5, y=210)

    mail_gender_btn=tk.Radiobutton(add_account_pages_fm, text="Male",
                                font=("bold", 12), variable=student_gender,
                                value="male")
    mail_gender_btn.place(x=5, y=235)

    femail_gender_btn=tk.Radiobutton(add_account_pages_fm, text="Female",
                                font=("bold", 12), variable=student_gender,
                                value="female")
    femail_gender_btn.place(x=75, y=235)

    student_gender.set("male")

    student_age_lb=tk.Label(add_account_pages_fm, text="Enter Student Age ",
                            font=("Bold", 12))
    student_age_lb.place(x=5, y=275)

    student_age_ent=tk.Entry(add_account_pages_fm, font=("Bold", 15),
                            highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    ...
    student_contact_lb=tk.Label(add_account_pages_fm, text="Enter Student Phone ",
                            font=("Bold", 12))
    student_contact_lb.place(x=5, y=360)
    

    student_contact_ent=tk.Entry(add_account_pages_fm, font=("Bold", 15),
                            highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    ...
    student_class_lb=tk.Label(add_account_pages_fm, text="Select Student Class ",
                            font=("Bold", 12))
    student_class_lb.place(x=5, y=445)

    select_class_btn=Combobox(add_account_pages_fm, font=("Bold", 15),
                            state="readonly",values=class_list)
    select_class_btn.place(x=5, y=475, width=180, height=30)

    student_id_lb=tk.Label(add_account_pages_fm, text=" Student ID Number: ",
                            font=("Bold", 12))
    student_id_lb.place(x=240, y=35)

    student_id=tk.Entry(add_account_pages_fm, font=("Bold", 18), bd=0)
    ...
    id_info_lb=tk.Label(add_account_pages_fm, text="""
    Automaticlly Generated ID Number!
    Remember Using This ID Number
    Student will Login Account.""", justify=tk.LEFT)
    id_info_lb.place(x=240, y=65)

    student_email_lb=tk.Label(add_account_pages_fm, text="Enter Student Emil Address",
                            font=("Bold", 12))
    student_email_lb.place(x=240, y=130)

    student_email_ent=tk.Entry(add_account_pages_fm, font=("Bold", 15),
                            highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    ...
    email_info_lb=tk.Label(add_account_pages_fm, text="""
    Via Email Address Student Can
    Recover Account! In Case Forgetting
    Password And Also Student will get 
    Future Notifications.""", justify=tk.LEFT)
    email_info_lb.place(x=240, y=190)


    account_password_lb=tk.Label(add_account_pages_fm, text="Create Account Password",
                            font=("Bold", 12))
    account_password_lb.place(x=240, y=275)

    account_password_ent=tk.Entry(add_account_pages_fm, font=("Bold", 15),
                            highlightbackground="gray",
                            highlightcolor=bg_color, highlightthickness=2)
    ...
    account_password_info_lb=tk.Label(add_account_pages_fm, text="""
    Via Student Created Password
    And Student ID Number 
    Student Can Login Account.""", justify=tk.LEFT)
    account_password_info_lb.place(x=240, y=345)

    home_btn=tk.Button(add_account_pages_fm, text="Home", font=("Bold", 15),
                            bg="red", fg="white", bd=0, command=forward_to_welcome_page)
    home_btn.place(x=240, y=420)

    student_btn=tk.Button(add_account_pages_fm, text="Submit", font=("Bold", 15),
                            bg=bg_color, fg="white", bd=0, command=check_input_validation)
    student_btn.place(x=360, y=420)

    add_account_pages_fm.pack(pady=5)
    add_account_pages_fm.pack_propagate(False)
    add_account_pages_fm.config(width=480, height=580)
